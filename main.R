...
if (!is.null(brentl)) {
    ...
    if (!is.null(brent)) {
        ...
        if (!is.null(urcb)) {
            ur <- getUsdRubRates(date_from, date_to)            
            if (!is.null(ur)) {    
                ...
                if (!is.null(uhnb)) {
                    ...
                    for (i in 1:temp) {
                        ...
                        if (nrow(new_df[new_df$Date == tmpd,]) != 0) {
                            new_df[new_df$Date == tmpd, "RateRub"] <- ur[i, "Close"]
                        } else {
                    ...
                    #
                    # Заполнение созданного data.frame данными о курсе ЦБ USD/RUB
                    #
                    temp <- nrow(urcb)
                    for (i in 1:temp) {
                        tmpd <- urcb[i, "Date"]
                        if (nrow(new_df[new_df$Date == tmpd,]) != 0) {
                            new_df[new_df$Date == tmpd, "RateRubCb"] <- urcb[i, "Close"]
                        } else {
                            new_df <- rbind(new_df, list(tmpd,
                                                      0.0, 0.0,
                                                      0.0, urcb[i, "Close"],
                    ...
                    for(i in 2:temp) {
                        ...
                        if (new_df[i, "RateRubCb"] == 0.0) new_df[i, "RateRubCb"] <- new_df[i-1, "RateRubCb"]
                    ...
                    if (new_df[1, "RateRubCb"] == 0.0) new_df[1, "RateRubCb"] <- new_df[2, "RateRubCb"]
                    ...
                    # 
                    tmpmin <- min(new_df$RateRubCb, new_df$RateRub)
                    tmpmax <- max(new_df$RateRubCb, new_df$RateRub)
                    plot(new_df$Date, new_df$RateRubCb,
                         type='l', col='blue',
                         main='USD - RUB Exchange Rate',
                         ylim=c(tmpmin, tmpmax),
                         xlab='2014', ylab='Price of 1 USD in RUB')
                    tmpmax = new_df[nrow(new_df), "RateRubCb"]
                    points(new_df$Date, replicate(nrow(new_df), tmpmax), type='l', lty='dotted', col='blue')
                    points(new_df$Date, new_df$RateRub, type='l', lty='dotted', col='cyan')
                    tmpmin = new_df[nrow(new_df), "RateRub"]
                    points(new_df$Date, replicate(nrow(new_df), tmpmin), type='l', lty='dotted', col='cyan')
                    # 
                    ...
                    tmpmin <- min(new_df$RateUah / new_df[1, "RateUah"],
                                 new_df$RateRubCb / new_df[1, "RateRubCb"])
                    ...
                    points(new_df$Date, replicate(nrow(new_df), tmpmax), type='l', lty='dotted', col='green')
                    points(new_df$Date, new_df$RateRubCb / new_df[1, "RateRubCb"],
                         type='l', col='blue')
                    tmpmin = new_df[nrow(new_df), "RateRubCb"] / new_df[1, "RateRubCb"]
                    ...
                    #
                    plot(new_df$Date, new_df$PriceBrent * new_df$RateRubCb,
                         type='l', col='red',
                         main='Crude Oil Brent Price',
                         xlab='2014', ylab='RUB / barrel')
                    points(new_df$Date, replicate(nrow(new_df), new_df[1, "PriceBrent"] * new_df[1, "RateRubCb"]),
                    ...
                    tmpmax <- brentl[nrow(brentl), "Close"]
                ...
                print("Ошибка при получении файла с курсом USD относительно RUB.")
            }
        } else {
